...
%   source : https://github.com/samuelpowell/toastpp/blob/master/script/matlab/toast2/toastLineSearch.m
%   modified on April 2017 (of --> cost)
%
...
verbose = strcmp(verbose,'true');

if isempty(p0)
    [p0,valid] =  get_of (x0, varargin{:});
end

% Init value and first step
...
if ph < pm
    ...
    if pmin > pm || ~valid || isnan(s) % no improvement or the new estimate
                                       % is nan
        s = sm;
        pmin = pm;
    end
